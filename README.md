...
- **Native on-device ML.** Apple Vision powers similar-photo grouping (`VNGenerateImageFeaturePrintRequest`) and closed-eye detection (`VNFaceDetectionRequest`). Similarity runs in time-windowed clusters to keep complexity bounded on large libraries.
...
- **iOS 15+ support.** The audience that needs cleanup tools the most is on older devices where storage pressure is real. The main app deliberately targets iOS 15.0 with availability guards around newer APIs, instead of dropping legacy users for the sake of newer SDK conveniences.
...
Swift, UIKit, SwiftUI, PhotoKit, Vision, StoreKit 2.
