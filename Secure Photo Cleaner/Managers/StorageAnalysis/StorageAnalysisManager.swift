...
final class StorageAnalysisManager {
    ...
    private let cacheFileURL: URL = {
        ...
        return docs.appendingPathComponent("SecurePhotoCleaner/storageAnalysisCache.json")
    }()
    private let progressFileURL: URL = {
        let docs = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
        return docs.appendingPathComponent("SecurePhotoCleaner/storageAnalysisProgress.json")
    }()

    private let chunkSize = 500

    private let stateQueue = DispatchQueue(label: "com.securephotocleaner.analysisState")
