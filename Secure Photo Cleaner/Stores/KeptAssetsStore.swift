...
final class KeptAssetsStore {
    static let shared = KeptAssetsStore()

    private let storageQueue = DispatchQueue(label: "com.galary.KeptAssetsStore.storage")
    private let fileURL: URL = {
        let docs = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
        return docs.appendingPathComponent("SecurePhotoCleaner/keptAssets.json")
    }()
